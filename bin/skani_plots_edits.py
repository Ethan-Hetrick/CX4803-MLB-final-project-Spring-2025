...
# --- New: Parse an optional output filename argument ---
output_filename = None
if len(sys.argv) > 2:
    if not sys.argv[2].startswith('-'): # Check if it's not another option like -vmax
        output_filename = sys.argv[2]
        # If output_filename was provided, vmax might be sys.argv[3]
        if len(sys.argv) > 3:
    ...
    else: # sys.argv[2] is likely vmax
        vmax = float(sys.argv[2])
        cg = seaborn.clustermap(square_mat, row_linkage = Z, col_linkage = Z, vmax = vmax, cmap = cmap)
else:
    cg = seaborn.clustermap(square_mat, row_linkage = Z, col_linkage = Z, cmap = cmap)

# --- Ensure a default output filename if none is provided ---
if output_filename is None:
    base_name = os.path.splitext(os.path.basename(file))[0]
    output_filename = f"{base_name}_clustermap.png"
...
# --- New: Save the figure ---
plt.savefig(output_filename, bbox_inches='tight', dpi=300) # dpi can be adjusted for resolution
print(f"Figure saved to {output_filename}")

# --- Optional: Show the plot (comment out if you only want to save) ---
# plt.show()
